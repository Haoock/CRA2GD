...
def get_all_paths(top_dir, excludes):
    """
    ...
    for (currentDir, _, filenames) in os.walk(top_dir):
...
def make_file_data(file_path, pkgCfg, timesSeen):
    """
    ...
    """
    bf = BuilderFile()
    bf.name = os.path.join(".", os.path.relpath(file_path, pkgCfg.scandir))
    filenameOnly = os.path.basename(file_path)
    bf.spdxID = getUniqueID(filenameOnly, timesSeen)
    (sha1, sha256, md5) = get_hashes(file_path)
    bf.sha1 = sha1
    if pkgCfg.doSHA256:
        bf.sha256 = sha256
    if pkgCfg.doMD5:
    ...
    expression = getExpressionData(file_path, pkgCfg.numLinesScanned)
    if expression is not None:
...
def make_package_data(pkg, timesSeen):
    """
    ...
    """
    filePaths = get_all_paths(pkg.config.scandir, pkg.config.excludeDirs)  # filePaths是每个文件的完整路径
    # bfs = make_all_file_data(filePaths, pkg.config, timesSeen)
    bfs = make_all_file_data(filePaths, pkg.config, timesSeen)
...
def makeDocument(docCfg):
    ...
    for pkg in doc.packages.values():
        make_package_data(pkg, timesSeen)
...
def makeFinalSpdx(srcRootDirs, spdxOutputDir, spdxNamespacePrefix):
    ...
    Arguments:
        - cm(deleted): Cmake codemodel parsed by parseReply()
...
if __name__ == "__main__":
    ...
    spdxPrefix = "https://swinslow.net/haoock/"
    srcRootDirs = {}
    pkgID = convertToSPDXIDSafe("test")
